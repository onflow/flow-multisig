echo "Creating account with cli and finoa keys for non-custodial"
echo "Creating tx using authorizer account in flow.json"
flow transactions build ./flow-multisig/scripts/acct-creation-cli-finoa.cdc \
  --network testnet \
  --args-json "$(cat "./flow-multisig/scripts/args-cli-finoa.json")" \
  --proposer $ACCT \
